...
export default function Header() {
  ...
  return (
    <header className="fixed top-2 left-1/2 transform -translate-x-1/2 w-[90%] z-50 p-3 lg:p-4 bg-white/30 backdrop-blur-md shadow-md rounded-4xl flex items-center mb-3  justify-between lg:grid lg:grid-cols-3 gap-x-32">
      {/* Logo */}
      <Link href="/" className="flex items-center gap-2">
        ...
        <h1 className="text-2xl font-bold text-green-400 hover:text-green-600 transition-all">
          Lazis<span className="text-green-600">ku</span>
        </h1>
      </Link>
      ...
      >
        {/* Link Navigasi */}
        <div className="flex flex-col lg:flex-row text-green-600 items-center lg:items-start justify-center lg:justify-center w-full space-y-4 lg:space-y-0 lg:space-x-6">
          {["Home", "About", "Services", "Program", "Contact"].map((item) => (
            <Link
              key={item}
              href={`/${
        ...
        <div className="flex flex-col items-center justify-center w-full lg:hidden gap-4 mt-2 flex-wrap">
          <Link href="/donasi">
